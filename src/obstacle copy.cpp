...
void SphereObstacle::initializeRendering(OpenGL::Rasterizer &r) {
  ...
  texBuf = r.createAttribBuf(texCoords);

  sphereObj = r.createObject();
  r.setVertexAttribs(sphereObj, vertexBuf);
  r.setVertexAttribs(sphereObj, normalBuf, 1);
  r.setVertexAttribs(sphereObj, texBuf, 2);
  r.setTriangleIndices(sphereObj, triangles);
...
void SphereObstacle::draw(OpenGL::Rasterizer &r, OpenGL::ShaderProgram &program,
  ...
  r.setUniform(program, "objectColor",
               glm::vec3(0.7f, 0.2f, 0.2f)); // Red sphere
  r.drawObject(sphereObj);
}
...
void PlaneObstacle::initializeRendering(OpenGL::Rasterizer &r) {
  ...
  r.setVertexAttribs(planeObj, vertexBuf);
  r.setVertexAttribs(planeObj, normalBuf, 1);
  r.setVertexAttribs(planeObj, texBuf, 2);
  r.setTriangleIndices(planeObj, triangles);
...
void PlaneObstacle::draw(OpenGL::Rasterizer &r, OpenGL::ShaderProgram &program,
  ...
  r.setUniform(program, "model", model);
  r.setUniform(program, "objectColor",
               glm::vec3(0.5f, 0.5f, 0.5f)); // Gray plane
  r.drawObject(planeObj);
}
